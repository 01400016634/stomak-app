...
import {
  ...
  ArrowUp, Save, ShoppingBag
} from 'lucide-react';

// --- DATA: INITIAL MENU (5 ITEMS PER CATEGORY) ---
...
export default function App() {
  ...
  const [bookingData, setBookingData] = useState({
    type: 'Table Reservation', guests: 2, date: '', description: '', menuType: 'Standard Menu', selectedMenuItems: []
  });
  ...
  const [newItemForm, setNewItemForm] = useState({ name: '', price: '', category: 'Burgers', details: '', image: '' });
  const [videoSource, setVideoSource] = useState("/Burger_and_Cola_Assembly_Video.mp4");
  ...
  const handleVideoUpload = (e) => {
    ...
    if (file) {
      ...
      showNotification("Site Branding Updated!");
    }
  };
  ...
  const addMenuItem = () => {
    ...
    setMenuItems([{ ...newItemForm, id, price: parseFloat(newItemForm.price) }, ...menuItems]);
    setNewItemForm({ name: '', price: '', category: 'Burgers', details: '', image: '' });
  ...
  const saveMenuManagerChanges = () => {
    showNotification("All Menu Changes Saved to Live Database!");
  };

  // --- CART LOGIC ---
  const addToCart = (item, cartType = 'customer') => {
    ...
    if (existing) {
      setTargetCart(targetCart.map(i => i.id === item.id ? { ...i, qty: i.qty + 1 } : i));
    } else {
      setTargetCart([...targetCart, { ...item, qty: 1 }]);
    }
    if (cartType === 'customer') showNotification(`Added ${item.name}!`);
  };
  ...
  const removeFromCart = (id, cartType = 'customer') => {
    const targetCart = cartType === 'customer' ? customerCart : posCart;
    const setTargetCart = cartType === 'customer' ? setCustomerCart : setPosCart;
    setTargetCart(targetCart.filter(item => item.id !== id));
  };
  ...
  const toggleMenuItemForBooking = (item) => {
    ...
    if (isSelected) setBookingData({ ...bookingData, selectedMenuItems: bookingData.selectedMenuItems.filter(i => i.id !== item.id) });
    else setBookingData({ ...bookingData, selectedMenuItems: [...bookingData.selectedMenuItems, item] });
  };

  const submitBookingRequest = () => {
    if (!bookingData.date || !bookingData.guests) {
      return showNotification("Please select a Date and number of Guests!", "error");
    }
    showNotification("Reservation Request Submitted Successfully!");
    // Reset booking form after successful submission
  ...
  const processOrderPayment = () => {
    ...
    const existingIdx = customerRecords.findIndex(c => c.phone === posCustomerInfo.phone);
    if (existingIdx !== -1) {
      const recordsCopy = [...customerRecords];
      recordsCopy[existingIdx] = {
        ...posCustomerInfo,
        isVIP: vipMembership || recordsCopy[existingIdx].isVIP,
        totalOrders: recordsCopy[existingIdx].totalOrders + 1,
        totalSpent: recordsCopy[existingIdx].totalSpent + posFinalTotal
      };
      setCustomerRecords(recordsCopy);
    } else {
      setCustomerRecords([...customerRecords, { ...posCustomerInfo, isVIP: vipMembership, totalOrders: 1, totalSpent: posFinalTotal }]);
    }
    ...
    showNotification("Payment Saved! Bill ready to print.");
  };
  ...
  const filteredCustomerMenu = activeCategory === 'All' ? menuItems : menuItems.filter(i => i.category === activeCategory);
  ...
  return (
    <div className="min-h-screen font-sans text-white bg-black selection:bg-orange-500/30 overflow-x-hidden print:bg-white print:text-black">
      ...
      <div className="fixed inset-0 z-0 overflow-hidden pointer-events-none print:hidden">
        <video key={videoSource} autoPlay loop muted playsInline className="w-full h-full object-cover opacity-50 scale-105"><source src={videoSource} type="video/mp4" /></video>
        ...
        <main className="pt-28 pb-32 px-4 md:px-6 max-w-7xl mx-auto min-h-screen">
          ...
          {currentView === 'home' && (
            <div className="h-[75vh] flex flex-col items-start justify-center animate-in fade-in slide-in-from-left-8 duration-1000">
              ...
              <div className="flex flex-wrap gap-4 w-full md:w-auto">
                ...
                <button onClick={() => setCurrentView('book')} className="w-full md:w-auto justify-center px-10 py-5 bg-white/5 rounded-full font-black text-lg border border-white/20 hover:bg-white/10 transition flex items-center gap-2">RESERVE TABLE <ChevronRight size={18} /></button>
          ...
          {currentView === 'menu' && (
            <div className="animate-in fade-in duration-500">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
                <div>
                  <h2 className="text-3xl md:text-4xl font-black mb-4 flex items-center gap-2"><Utensils className="text-orange-500" /> Our Menu</h2>
                  <div className="bg-white/5 backdrop-blur-md p-1.5 rounded-2xl flex flex-wrap gap-1 border border-white/10 w-fit">
              ...
              <div className="flex flex-wrap gap-2 md:gap-3 mb-10 selection:bg-none">
                {categoryList.map(cat => (
                  <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-4 md:px-6 py-2 rounded-full font-bold text-sm md:text-base border transition-all ${activeCategory === cat ? 'bg-white text-black border-white' : 'border-white/20 text-gray-300 hover:border-orange-500'}`}>{cat}</button>
                ))}
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredCustomerMenu.map(item => (
                  <div key={item.id} className="bg-white/5 rounded-3xl p-5 md:p-6 border border-white/10 hover:border-orange-500/50 transition duration-300 flex gap-4 md:gap-5 overflow-hidden">
                    ...
                    <div className="flex flex-col flex-1">
                      ...
                      <p className="text-xs text-gray-400 mb-2 flex-1 line-clamp-2">{item.details}</p>
                      <div className="flex justify-between items-center mt-auto pt-3 border-t border-white/10">
                        ...
                        <button onClick={() => addToCart(item)} className="h-8 w-8 md:h-10 md:w-10 bg-white/10 rounded-full flex items-center justify-center hover:bg-orange-600 transition"><Plus size={18} /></button>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {currentView === 'book' && (
            <div className="max-w-4xl mx-auto bg-black/60 p-6 md:p-10 rounded-3xl md:rounded-[40px] border border-white/10 backdrop-blur-xl animate-in slide-in-from-bottom-10 flex flex-col lg:flex-row gap-8">
              ...
              {/* Standard Menu Selector Modal Trigger */}
          ...
          {currentView === 'admin' && (
            <div className="flex flex-col lg:flex-row gap-6 md:gap-8 min-h-[75vh]">
              ...
              {/* Admin Content Area */}
              <div className="flex-1 bg-black/60 backdrop-blur-xl border border-white/10 rounded-3xl md:rounded-[40px] p-5 md:p-8 flex flex-col h-[70vh] lg:h-auto overflow-hidden">
                ...
                {adminTab === 'pos' && (
                  <div className="flex flex-col lg:flex-row gap-6 h-full">
                    ...
                    <div className="flex-1 overflow-y-auto pr-2 md:pr-4 space-y-3">
                      ...
                      {menuItems.map(item => (
                        <div key={item.id} onClick={() => addToCart(item, 'pos')} className="flex items-center gap-3 md:gap-4 bg-white/5 p-3 rounded-2xl cursor-pointer hover:bg-orange-500/20 border border-transparent transition">
                    ...
                    <div className="w-full lg:w-[350px] bg-black/50 rounded-3xl p-5 flex flex-col border border-white/10 shrink-0 h-96 lg:h-auto">
                      ...
                      <div className="space-y-2 mb-4 border-b border-white/10 pb-4">
                        <input type="text" placeholder="* Name" value={posCustomerInfo.name} onChange={e => setPosCustomerInfo({ ...posCustomerInfo, name: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white outline-none focus:border-orange-500 text-sm" />
                        <input type="tel" placeholder="* Phone" value={posCustomerInfo.phone} onChange={e => setPosCustomerInfo({ ...posCustomerInfo, phone: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white outline-none focus:border-orange-500 text-sm" />
                      </div>
                      ...
                      <div className="flex-1 overflow-y-auto space-y-3 mb-4 border-b border-white/10 pb-4">
                        {posCart.length === 0 ? <p className="text-gray-500 text-center text-sm mt-4">Cart is empty</p> :
                          posCart.map((item, idx) => (
                            <div key={idx} className="flex gap-2 text-xs md:text-sm items-center">
                      ...
                      <button onClick={processOrderPayment} className="w-full py-3 bg-white text-black rounded-xl font-black text-sm md:text-base hover:bg-gray-200 transition">CHARGE & PRINT</button>
                    </div>
                  </div>
                )}
                ...
                {adminTab === 'menuManager' && (
                  <div className="flex-1 overflow-hidden flex flex-col space-y-6">
                    ...
                    <div className="bg-white/5 p-4 md:p-6 rounded-3xl border border-white/10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 items-end">
                      ...
                      <div><input type="text" placeholder="Image URL" value={newItemForm.image} onChange={e => setNewItemForm({ ...newItemForm, image: e.target.value })} className="w-full bg-black/40 border border-white/10 rounded-xl p-3 text-sm text-white" /></div>
                    ...
                    <div className="pt-4 border-t border-white/10 flex justify-end">
                      <button onClick={saveMenuManagerChanges} className="py-3 px-8 bg-green-600 rounded-xl font-black flex items-center gap-2 hover:bg-green-500 transition"><Save size={18} /> SAVE ALL CHANGES</button>
                    </div>
                  </div>
                )}

                {adminTab === 'records' && (
                  <div className="flex-1 overflow-auto"><h3 className="text-2xl font-black mb-4">Customer Records ({customerRecords.length})</h3>
                    <div className="text-sm text-gray-400 text-center mt-10 border border-dashed border-white/10 p-10 rounded-2xl">Records populate here after checkout.</div></div>
                )}
        ...
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-md p-4 flex items-center justify-center animate-in fade-in print:hidden">
          <div className="bg-black/90 w-full max-w-md rounded-3xl border border-white/10 p-6 flex flex-col">
            <div className="flex justify-between items-center mb-6 border-b border-white/10 pb-4">
              <h4 className="text-xl font-black flex items-center gap-2"><ShoppingCart className="text-orange-500" /> Your Order ({orderType})</h4>
            ...
            <button onClick={() => { setShowCustomerCheckout(false); showNotification('Order Sent to Kitchen!'); setCustomerCart([]); }} className="w-full py-4 bg-orange-600 rounded-xl font-black hover:bg-orange-500 transition shadow-lg">PLACE ORDER NOW</button>
          </div>
        </div>
      )}
      ...
      {receiptData && (
        <div className="fixed inset-0 z-[300] bg-black/80 backdrop-blur-md p-4 flex items-center justify-center animate-in fade-in print:absolute print:bg-white print:text-black print:p-0">
          <div className="bg-white text-black w-full max-w-sm rounded-3xl p-8 flex flex-col shadow-2xl print:shadow-none print:w-full print:rounded-none">
            ...
            <div className="space-y-2 mb-6 text-sm font-bold border-b border-dashed border-gray-300 pb-4">
